...
def step2_learnig() :
    ...
    # 학습이 완료된 객체를 저정한다.
    with open('./5.Scikit-Logistic/ml.dat', 'wb') as fp :
        pickle.dump(sc, fp)
        pickle.dump(ml, fp)
    ...
    # plot_decision_region(X=X_combined_std, y=y_combined_std, classifier=ml, test_idx=range(105, 150), title='perceptron')

def step3_using() :
    # 학습이 완료된 객체를 복원한다.
    with open('./5.Scikit-Logistic/ml.dat', 'rb') as fp :
        sc = pickle.load(fp)
        ml = pickle.load(fp)
